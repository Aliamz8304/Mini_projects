...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=5) # random_state is the seed 
...
new_days_scaled = scalar.transform(new_days_df)
...
plt.scatter(new_days, new_prices, color='yellow', label = 'Predicting Data')
plt.plot(X_train, y_pred_train, color='brown', label = 'Training Prediction')
plt.plot(X_test, y_pred_test, color='black', label = 'Testing Prediction')
plt.xlabel('Days')
plt.ylabel('Prices')
