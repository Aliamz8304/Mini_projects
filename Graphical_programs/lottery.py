    ...
    try:
        with open(file_path, "r") as file:
            ...
            if len(name_list) < num:
                messagebox.showwarning("عدد اشتباه", "عدد انتخاب شده بیشتر از تعداد افراد است!")
                return
            winners_list = random.sample(name_list, num)
            top_window = tk.Toplevel()
            top_window.title("لیست برندگان")
            top_window.geometry("400x600")
            top_window.configure(background="#7FFFD4")
...
window.configure(background="#0B192C")

file_label =ttk.Label(window, text="فایل شرکت کنندگان را انتخاب کنید",
                      font=("Arial", 14), background="#A1E3F9", foreground="#000000").pack(pady=20) 

style = ttk.Style()
style.configure("TFrame", background="#E50046")
...
select_button = ttk.Button(window, text="انتخاب برندگان", command=select_winners).pack(pady=5)
